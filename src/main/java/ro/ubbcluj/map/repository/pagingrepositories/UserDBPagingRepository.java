...
public class UserDBPagingRepository extends UserDBRepository implements PagingRepository<String, Utilizator> {
    ...
    public UserDBPagingRepository(Connection connection) {
        super(connection);
    }
    ...
    public Page<Utilizator> findAll(Pageable pageable, String id) {
        ...
        try (PreparedStatement statement = connection.prepareStatement("select * from users where username != ? limit ? offset ?")
        ) {
